...
Properties {
	...
	[NoScaleOffset] _StarsCube("Stars", Cube) = "black" {}
	[Gamma] _StarsExposure ("Stars Exposure", Range(0, 8)) = 1.0
}
...
SubShader {
	...
	Pass {
		...
		half4 _StarsCube_HDR;
		uniform half _StarsExposure;
		...
		v2f vert (appdata_t v)
		{
            ...
            OUT.pos = UnityObjectToClipPos(mul(_Rotation, v.vertex));
			OUT.cubeCoords = v.vertex;
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		{
			half4 stars = texCUBE( _StarsCube, IN.cubeCoords );
			stars.rgb = DecodeHDR( stars, _StarsCube_HDR );
			half3 col = stars * (1.0- _AtmosphereThickness) * _StarsExposure;
